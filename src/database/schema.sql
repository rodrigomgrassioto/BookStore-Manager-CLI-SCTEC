...
COMMENT ON COLUMN emprestimos.status IS 'Valores aceitos: ATIVO, DEVOLVIDO, ATRASADO';

-- Chaves Estrangeiras limpas e sem excessos
ALTER TABLE livros ADD FOREIGN KEY (id_autor) REFERENCES autores (id_autor) ON DELETE CASCADE;
ALTER TABLE emprestimos ADD FOREIGN KEY (id_livro) REFERENCES livros (id_livro);
ALTER TABLE emprestimos ADD FOREIGN KEY (id_cliente) REFERENCES clientes (id_cliente);
